  ...
  def render(assigns) do
    ...
    >
      <div class="w-full h-1 bg-gray-200 rounded">
        ...
        rounded after:content-[''] after:w-2 after:h-2 after:rounded after:absolute after:top-[-2px] after:right-[-2px] after:bg-orange-600 transition-width duration-700 ease-out"
          style={"width: #{if @current_step + 1 === Enum.count(@steps), do: "100%", else: "calc("<> Float.to_string(100 / Enum.count(@steps) * (@current_step + 1)) <> "% - " <> Float.to_string(100 / Enum.count(@steps) / 2) <> "%)"}"}
        >
