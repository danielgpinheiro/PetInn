defmodule PetInnWeb.CheckoutLive do
  ...
  def render(assigns) do
    ...
    <section id="confirmation" class="w-full h-full bg-white dark:bg-gray-800 relative">
      ...
      <div :if={
        @inn.ok? && @inn.result && @user.ok? && @user.result && @booking.ok? && @booking.result
      }>
      ...
      <div :if={
        (@inn.ok? && @inn.result === nil) || (@user.ok? && @user.result === nil) ||
          (@booking.ok? && @user.result === nil)
      }>
  ...
  def mount(params, _session, socket) do
    ...
    booking_id = Map.fetch!(params, "booking_id")
    ...
    {:ok,
     ...
     |> assign_async(:booking, fn -> {:ok, %{booking: BookingController.get_by_booking_id(booking_id)}} end)
